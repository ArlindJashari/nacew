...
import { useNavSurfaceTheme } from "../useNavSectionTheme";
...
function NavCta({ children }: { children: string }) {
  return (
    <ContactTrigger as="a" className="nacew-nav-cta">
      {children}
    </ContactTrigger>
  );
}
...
function DesktopNav({ backdropOpacity }: { backdropOpacity: number }) {
  return (
    <nav className="nacew-nav nacew-nav--desktop" data-nacew-theme="dark" aria-label="Primary">
      <motion.div
...
export function Nav() {
  ...
  const surfaceTheme = useNavSurfaceTheme();
  const backdropOpacity = Math.max(scrollBackdropOpacity, surfaceTheme === "light" ? 1 : 0);
  ...
  return (
    ...
    >
      <DesktopNav backdropOpacity={backdropOpacity} />
      <MobileNav
